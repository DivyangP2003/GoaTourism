...
export default function NonRecognisedSiteDetailPage() {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-black mb-4">Non Recognised Site Not Found</h1>
          <Link href={`/archaeological-sites/unprotected-sites/${districtSlug}`}>
            <button className="bg-primary text-black px-6 py-2 rounded-full font-semibold hover:bg-secondary hover:text-white transition-all duration-200">
              Back to {district?.name || "District"} Non Recognised Sites
            </button>
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="flex items-center space-x-2 text-sm text-gray-600 mb-12">
          ...
          <ChevronRight className="h-4 w-4" />
          <Link href={`/archaeological-sites/unprotected-sites/${districtSlug}`} className="hover:text-primary transition-colors duration-200">
            {district.name}
          </Link>
          <ChevronRight className="h-4 w-4" />
        ...
        <div className="bg-gray-50 rounded-2xl p-8 mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-700 text-sm">
                Follow Non Recognised guidelines, no touching of monuments, photography may be restricted
              </p>
            </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link href={`/archaeological-sites/unprotected-sites/${districtSlug}`}>
            <button className="bg-primary text-black px-8 py-3 rounded-full font-semibold text-lg hover:bg-secondary hover:text-white transition-all duration-200">
              ← Back to {district.name} Non Recognised Sites
            </button>
          </Link>
          <Link href="/contact">
