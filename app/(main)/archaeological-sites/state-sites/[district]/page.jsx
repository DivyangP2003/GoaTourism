...
export default function StateSitesDistrictPage() {
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="relative h-64 overflow-hidden mb-16">
        ...
        <div className="relative z-10 flex items-center justify-center h-full">
          <h1 className="text-4xl md:text-6xl font-bold text-white tracking-wider drop-shadow-lg text-center">
            STATE SITES IN {district.name.toUpperCase()}
          </h1>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
          {sites.map((site) => (
            <Link key={site.id} href={`/archaeological-sites/state-sites/${districtSlug}/${site.slug}`}>
        ...
        <div className="mt-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {otherDistricts.map((otherDistrict) => (
              <Link key={otherDistrict.id} href={`/archaeological-sites/state-sites/${otherDistrict.slug}`}>
                <div className="group cursor-pointer">
                  {/* District Box */}
                  <div
                  ...
                  >
                    ...
                    <div className="relative z-10 text-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      ...
                      <span className="text-sm text-white font-bold">
                        {stateSitesByDistrict[otherDistrict.slug]?.length || 0} State Sites
                      </span>
                    </div>
                  </div>
